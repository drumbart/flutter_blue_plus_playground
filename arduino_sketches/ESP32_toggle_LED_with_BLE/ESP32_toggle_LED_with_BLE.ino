...
void setup() {
  ...
  pRedChar = pService->createCharacteristic(RED_LED_CHAR_UUID, BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_WRITE | BLECharacteristic::PROPERTY_NOTIFY);
  pRedChar->addDescriptor(new BLE2902());
  pRedChar->setCallbacks(new LedWriteCallback(redLedPin));

  pGreenChar = pService->createCharacteristic(GREEN_LED_CHAR_UUID, BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_WRITE | BLECharacteristic::PROPERTY_NOTIFY);
  pGreenChar->addDescriptor(new BLE2902());
  pGreenChar->setCallbacks(new LedWriteCallback(greenLedPin));

  pYellowChar = pService->createCharacteristic(YELLOW_LED_CHAR_UUID, BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_WRITE | BLECharacteristic::PROPERTY_NOTIFY);
...
void loop() {
  checkButton(redButtonPin, redLedPin, redLedState, redButtonPressed, lastDebounceTimeRed, pRedChar);
  checkButton(greenButtonPin, greenLedPin, greenLedState, greenButtonPressed, lastDebounceTimeGreen, pGreenChar);
  checkButton(yellowButtonPin, yellowLedPin, yellowLedState, yellowButtonPressed, lastDebounceTimeYellow, pYellowChar);
}

void checkButton(int buttonPin, int ledPin, bool &ledState, bool &buttonPressed, unsigned long &lastDebounce, BLECharacteristic* characteristic) {
